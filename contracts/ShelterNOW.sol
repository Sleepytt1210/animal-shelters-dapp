...
contract ShelterNOW is IERC20, Context, Ownable {
  ...
  */
  ...
  function _transferStandard(address sender, address recipient, uint256 tokenAmount) private {
    (uint256 reflectionAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tokenAmount);
    _rOwned[sender] = _rOwned[sender] - reflectionAmount;
  ...
  function _getValues(uint256 tokenAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
    (uint256 tTransferAmount, uint256 tFee) = _getTValues(tokenAmount);
    uint256 currentRate =  _getRate();
    (uint256 reflectionAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tokenAmount, tFee, currentRate);
    return (reflectionAmount, rTransferAmount, rFee, tTransferAmount, tFee);
  }

  function _getTValues(uint256 tokenAmount) private view returns (uint256, uint256) {
    uint256 tFee = (tokenAmount * _taxFee) / 100;
    uint256 tTransferAmount = tokenAmount - tFee;
    return (tTransferAmount, tFee);
  }

  function _getRValues(uint256 tokenAmount, uint256 tFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
  ...
  */
  function calculateGrossAmount(uint256 netAmount) public view returns(uint256) {
  ...
  * @return The gross amount required.
  */
  function calculateNetAmount(uint256 grossAmount) public view returns(uint256) {
